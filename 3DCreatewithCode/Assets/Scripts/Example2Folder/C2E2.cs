...
public class C2E2 : MonoBehaviour
{
...
public class Mover2_2
{
    ...
    public void CheckBoundaries()
    {
        ...
        {
            Vector3 wind1 = new Vector3(.05f, 0f, 0f);
            body.AddForce(wind1, ForceMode.Impulse);

            restrainedVelocity.x = Mathf.Abs(restrainedVelocity.x);     //Add in forces here as winds - or + in order to "keep in bounds".
        }
        else if (body.position.x + radius > xMax)
        {
            Vector3 wind2 = new Vector3(-.05f, 0f, 0f);
            restrainedVelocity.x = -Mathf.Abs(restrainedVelocity.x);
            body.AddForce(wind2, ForceMode.Impulse);
        }
